...
def get_mapping():
    return {
        "http://127.0.0.1:45200": "genesis",
        "http://127.0.0.1:45201": "pr1",
        "http://127.0.0.1:45202": "pr2",
        "http://127.0.0.1:45203": "pr3",
...
def find_overlap(election_overlap, active_elections):
    ...
    if "http://192.168.178.88:45000" in active_elections:
...
def main_gather_elections(interval=5, expected_started_elections=1000):
    ''' Gathers started elections every 5s {interval}       
        if started_elections_count matches {expected_started_elections} electiond etails are dumpted into sqlite database

        if no new active elections occur between runs, "active_elections" is called and the result dumped to file        
        Generates a .json file that contains the information from the active_elections rpc (and the other defined)
        generated file is named ./{run_id}/HH:MM:SS_election_started.json'''
...
def parse_args():
    ...
    parser.add_argument('-w', '--wait', type=int, help='interval')
    parser.add_argument('-E',
                        '--expected',
                        type=int,
                        help='expected election count')
...
if __name__ == "__main__":
    ...
    if args.command == "gather":
        main_gather_elections(args.wait, args.expected)
