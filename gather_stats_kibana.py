    ...
    async def post_requests_parallel(self, data, sync = True, aio_results = [], include_request = False, ignore_errors = [], aio_errors=None):
        ...
        for auth, requests in data.items() :
            ...
            async with aiohttp.ClientSession(auth=auth) as session:
                        ...
                        try:                
                            async with session.post(url=el["url"], json=el["request"], ssl=False) as response:                            
                                ...
                                if obj is None :
                                    ...
                                    aio_errors["last_error"] = f'Request failed: {el}'                                   
                                if "error" in obj :
                ...
                await asyncio.gather(*(do_req(el, aio_errors) for el in requests))
...
class NanoStats():
    ...
    def extend_node_stats_all_nodes(self, nodes_stats, timestamp) :     #prepare_node_stats step2 
        ...
        for action, urls in request_responses.items():            
            for url, response in urls.items():
                ...
                if action == "stats" : 
                    ...
                    for key,value in self.values_to_integer(response, skip_keys=["node_vendor", "node_store", "build_info", ]).items(): 
    ...
    def collect_requests(self): #compare_active_elections step1
        requests = {}
        for rpc in self.rpcs:
            auth = rpc["rpc_auth"] if "rpc_auth" in rpc else {"user" : None, "password" : None}
            auth_key = f'{auth["user"]}|{auth["password"]}'
            if auth_key not in requests : requests[auth_key] = []
            requests[auth_key].append(self.get_req_object(rpc["rpc_url"], {"action" : "stats","type" : "counters"}))
            requests[auth_key].append(self.get_req_object(rpc["rpc_url"], {"action" : "block_count"}))
            requests[auth_key].append(self.get_req_object(rpc["rpc_url"], {"action" : "confirmation_active"}))
            requests[auth_key].append(self.get_req_object(rpc["rpc_url"], {"action" : "confirmation_quorum"}))
            requests[auth_key].append(self.get_req_object(rpc["rpc_url"], {"action" : "version"}))
        return requests
